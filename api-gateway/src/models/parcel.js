...
/**
 * Get a parcel by ID with row lock for update (FOR UPDATE)
 * Used in transaction-based operations to prevent concurrent modifications
 */
 ...
 * Update parcel owner
 */
async function updateParcelOwner(parcelId, newOwnerId, client) {
  const query = `
    UPDATE parcels
    SET owner_id = $1
    WHERE parcel_id = $2
  ...
  `;
  const result = await client.query(query, [newOwnerId, parcelId]);
